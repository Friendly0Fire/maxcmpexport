...
{
	HTREEITEM self = tree->InsertItem(stows(name).c_str(), 0, 0, parent);
...
void UTF::insert_nodes(CTreeCtrl * tree, NODE * node, int node_size, char * string, char * data, HTREEITEM parent_item, int offset)
{
	...
	while (aux_node != node)
	{
		HTREEITEM self = tree->InsertItem(stows(aux_node->string_offset + string).c_str(), 0, 0, parent_item);
		node_list->push_back(self);
		...
		else
		{
			...
			if (aux_node->allocated_size > 0)
			{
				...
				if (aux_node->size != aux_node->size2)
					MessageBox(0, _T("Possible compression being used on this chunk"), stows(string + aux_node->string_offset).c_str(), 0);
...
{
	HTREEITEM root_item = tree->InsertItem(stows(string + node->string_offset).c_str(), 0, 0);
