...
BOOL cDlgOptions::OnInitDialog() 
{
	...
	for (std::list<MMESH*>::iterator i = meshList->begin();i != meshList->end(); ++i)
	{
		...
		HTREEITEM MatData = utf->AddNewNode(tree, MaterialLib, MatName);
			...
			*(size_t *)CR_Type_data = strlen(TypeName) + 1;
			...
			else if(bIsSelfIlluminate)
			{
				...
				strcpy (SIMatName, mesh->SIname);
				strcat (MatName, sMat_Name.c_str());
				strcpy (MatFilePath, mesh->pFilePath);
				strcat (SIMatName, sSiMat_Name.c_str());
				strcpy (SIMatFilePath, mesh->pSIFilePath);
				...
				*(size_t *)DtName_data = strlen(MatName) + 1;
				...
				*(size_t *)EtName_data = strlen(SIMatName) + 1;
				...
				HTREEITEM Texture2 = utf->AddNewNode(tree, TextureLib, SIMatName);
					  ...
					  if(fn.substr(fn.find_last_of(".") + 1) == "DDS") 
					  {    
					  		HTREEITEM MIPType = utf->AddNewNode(tree, Texture, "MIPS");
					  ...
					  else if(fn.substr(fn.find_last_of(".") + 1) == "dds") 
					  {    
				  			HTREEITEM MIPType = utf->AddNewNode(tree, Texture, "MIPS");
					  ...
					  {    
							HTREEITEM MIPType = utf->AddNewNode(tree, Texture, "MIP0");
					  ...
					  if(fn2.substr(fn2.find_last_of(".") + 1) == "DDS") 
					  {    
					  		HTREEITEM MIPType = utf->AddNewNode(tree, Texture2, "MIPS");
					  ...
					  else if(fn2.substr(fn2.find_last_of(".") + 1) == "dds") 
					  {    
				  			HTREEITEM MIPType = utf->AddNewNode(tree, Texture2, "MIPS");
					  ...
					  {    
							HTREEITEM MIPType = utf->AddNewNode(tree, Texture2, "MIP0");
