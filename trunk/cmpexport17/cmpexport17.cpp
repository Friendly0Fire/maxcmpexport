...
#define cmpexport17_CLASS_ID	Class_ID(0x690d83a4, 0x7434c235)
...
class cmpexport17 : public SceneExport {
public:
...
class cmpexport17ClassDesc : public ClassDesc2 
{
public:
	virtual int IsPublic() 							{ return TRUE; }
	virtual void* Create(BOOL /*loading = FALSE*/) 		{ return new cmpexport17(); }
	virtual const TCHAR *	ClassName() 			{ return GetString(IDS_CLASS_NAME); }
	virtual SClass_ID SuperClassID() 				{ return SCENE_EXPORT_CLASS_ID; }
	virtual Class_ID ClassID() 						{ return cmpexport17_CLASS_ID; }
	virtual const TCHAR* Category() 				{ return GetString(IDS_CATEGORY); }

	virtual const TCHAR* InternalName() 			{ return _T("cmpexport17"); }	// returns fixed parsable name (scripter-visible name)
	virtual HINSTANCE HInstance() 					{ return hInstance; }					// returns owning module handle
	

};

ClassDesc2* Getcmpexport17Desc() { 
	static cmpexport17ClassDesc cmpexport17Desc;
	return &cmpexport17Desc; 
}

INT_PTR CALLBACK cmpexport17OptionsDlgProc(HWND hWnd,UINT message,WPARAM,LPARAM lParam) {
	static cmpexport17* imp = nullptr;

	switch(message) {
		case WM_INITDIALOG:
			imp = (cmpexport17 *)lParam;
...
int cmpexport17::ExtCount()
{
	//#pragma message(TODO("Returns the number of file name extensions supported by the plug-in."))
	return 1;
}

const TCHAR *cmpexport17::Ext(int /*i*/)
{		
	//#pragma message(TODO("Return the 'i-th' file name extension (i.e. \"3DS\")."))
	return _T("cmp");
}

const TCHAR *cmpexport17::LongDesc()
{
	//#pragma message(TODO("Return long ASCII description (i.e. \"Targa 2.0 Image File\")"))
	return _T("Freelancer CMP File Format");
}
	
const TCHAR *cmpexport17::ShortDesc() 
{			
	//#pragma message(TODO("Return short ASCII description (i.e. \"Targa\")"))
	return _T("cmp");
}

const TCHAR *cmpexport17::AuthorName()
{			
	//#pragma message(TODO("Return ASCII Author name"))
	return _T("w0dk4 & SilentAssassin82");
}

const TCHAR *cmpexport17::CopyrightMessage() 
{	
	//#pragma message(TODO("Return ASCII Copyright message"))
	return _T("");
}

const TCHAR *cmpexport17::OtherMessage1() 
{		
	//TODO: Return Other message #1 if any
	return _T("");
}

const TCHAR *cmpexport17::OtherMessage2() 
{		
	//TODO: Return other message #2 in any
	return _T("");
}

unsigned int cmpexport17::Version()
{				
...
bool cmpexport17::NewNode(INode* node, IGameNode * mNode)
{
	...
	if (numSubs == 1)
	{
		...
		theHold.Accept(L"Mesh Split By MatID");
	}
	...
	for (int i = 0; i<material->NumSubMtls(); i++)
	{
		...
		// Set the transform:
		Matrix3 m3 = node->GetNodeTM(ip->GetTime());
		newNode->SetNodeTM(ip->GetTime(), m3);
...
int cmpexport17::RegroupByMatID()
{
	IGameScene* nMeshes;
...
bool cmpexport17::CreateCMPData(IGameNode * pRootGrp, list<CMPND_DATA*>* lstCMPData)
{
	...
	cmpnd->index = 0;
	cmpnd->sObjectName = wstos(pRootGrp->GetName());
	...
	for (int nNodeLOD = 0; nNodeLOD < pRootGrp->GetChildCount(); nNodeLOD++)
	{
		...
		{
			// get lod number
			iLOD = ((int)cur_lodnode->GetName()[(wstos(cur_lodnode->GetName()).size() - 1)]) - 48;
			if (iLOD >(MAX_LODS - 1))
			{
				MessageBox(0, _T("You have specified a higher LOD number than supported (0 to 6)"), _T("Error exporting LOD"), MB_ICONERROR);
				continue; // ignore
			}
			if (wstos(cur_lodnode->GetName()).substr(0, 3) == "vwd")
				cmpnd->object_data->iLODWireframe = iLOD;

		}
		else
		{
			MessageBox(0, _T("The node under your component must specify the lod."), _T("Error exporting LOD"), MB_ICONERROR);
			continue; // ignore
		}
		...
		for (int nNode = 0; nNode < cur_lodnode->GetChildCount(); nNode++)
		{
			...
			if (cur_node->GetIGameObject()->GetIGameType() == IGameObject::IGAME_MESH)
			{
				...
				if (pMaterial == NULL)
				{
					MessageBox(0, _T("No material on one of the meshes, adding default name"), _T("Error exporting Material"), MB_ICONERROR);
					mesh->sMaterial = "default";
				}
				else
				{
					mesh->sMaterial = wstos(pMaterial->GetMaterialName());
					if (pMaterial->GetSubMaterialCount())
						MessageBox(0, _T("Freelancer does not support sub-materials!"), _T("Error exporting Material"), MB_ICONERROR);
				}

				// mesh name
				mesh->sName = wstos(cur_node->GetName());
		...
		{
			wstring wsError = L"The group \"";
			wsError += stows(cmpnd->sObjectName);
			wsError += L"\" references more vertices than Freelancer can handle. Split the group into smaller groups!";
			MessageBox(0, wsError.c_str(), _T("Error exporting a group object"), MB_ICONERROR);
			return false;
		}
...
void cmpexport17::check_line(Line *lines, uint *num_lines, Line new_line) {
	uint i;
...
void cmpexport17::CreateVWData(CMPND_DATA* cmpnd)
{
	...
	if (iTotalVWireIndices > 16000)
	{
		...
		wstring wsError = L"Warning: Your total indices count for the Wireframe exceeds Freelancer's limit of 16 000. Indices: ";
		wsError += to_wstring(iTotalVWireIndices);
		MessageBox(0, wsError.c_str(), _T("Warning exporting VWireData"), MB_ICONERROR);
...
bool cmpexport17::ExportGroup(IGameNode * pRootGrp, string sExportFilename)
{
...
int	cmpexport17::DoExport(const TCHAR *export_filename, ExpInterface * /*ei*/, Interface *i, BOOL /*suppressPrompts*/, DWORD options)
{
	...
	for (int nMesh = 0; nMesh < nMeshes->GetTopLevelNodeCount(); nMesh++)
	{
		...
		if (pMesh->IsGroupOwner() && !wcscmp(pMesh->GetName(), L"Root"))
		{
			if (bFoundRoot)
			{
				MessageBox(0, _T("You have more than one node with the name \"Root\"! One Root node has been picked as Root, however this might not be the intended one!"), _T("Warning exporting Root node."), MB_ICONERROR);
				return 1;
			}

			if (!ExportGroup(pMesh, wstos(export_filename)))
				return 1;
			bFoundRoot = true;
		}
	}
	...
	if (!bFoundRoot)
	{
		MessageBox(0, _T("You need to have one group named \"Root\" to export!"), _T("Error finding Root node."), MB_ICONERROR);
		return 1;
	}
...
BOOL cmpexport17::SupportsOptions(int ext, DWORD options) {
	assert(ext == 0);	// We only support one extension
	return(options == SCENE_EXPORT_SELECTED) ? TRUE : FALSE;
}

TSTR cmpexport17::GetCfgFilename()
{
	TSTR filename;
